...
class ISPController extends Controller
{
    ...
    public function dashboard(): View
    {
        ...
        // ISP's MRC (all customers)
        $lastMonth = now()->subMonth();
        $ispMRC = [
            'current_mrc' => $calculateCurrentMRC(),
            'this_month_avg_mrc' => $calculateMonthlyAvgMRC(now()->year, now()->month),
            'last_month_avg_mrc' => $calculateMonthlyAvgMRC($lastMonth->year, $lastMonth->month),
        ];
        ...
        return view('panels.isp.dashboard', compact(
            'stats',
            'statusDistribution',
            'expiringCustomers',
    ...
    public function users(): View
    {
        ...
        return view('panels.isp.users.index', compact('users'));
    }
    ...
    {
        return view('panels.isp.users.create');
    }
    ...
    public function usersStore(Request $request)
    {
        ...
        return redirect()->route('panel.isp.users')
    ...
    public function usersEdit($id): View
    {
        ...
        return view('panels.isp.users.edit', compact('user'));
    }
    ...
    */
    ...
    public function routersStore(Request $request)
    {
        $validated = $request->validate([
            ...
            'password.min' => 'API password must be at least 4 characters.',
     ...
     */
    /**
     * Drill-down OLT details: ONU list with signal strength, resource/health, quick links.
     */
    public function oltDetails(int $id): View
    {
        $olt = Olt::with(['onus'])->where('tenant_id', getCurrentTenantId())->findOrFail($id);

        $onus = $olt->onus()->orderBy('pon_port')->orderBy('onu_id')->get();

        $signalStats = [
            'ok' => $onus->where('signal_rx', '>=', -27)->where('signal_rx', '<=', -8)->count(),
            'low' => $onus->filter(fn ($o) => $o->signal_rx !== null && $o->signal_rx < -27)->count(),
    ...
    /**
     * Drill-down router details: resource monitor, active sessions, quick links.
     */
    public function mikrotikDetails($id): View
    {
        $router = MikrotikRouter::with(['nas'])
            ->where('tenant_id', getCurrentTenantId())
            ->findOrFail($id);

        $deviceMonitor = \App\Models\DeviceMonitor::where('monitorable_type', 'App\Models\MikrotikRouter')
            ->where('monitorable_id', $router->id)
            ->first();

        $activeSessions = [];
        try {
            $api = app(\App\Services\MikrotikApiService::class);
            $activeSessions = $api->getMktRows($router, 'ppp/active', []);
        } catch (\Throwable $e) {
