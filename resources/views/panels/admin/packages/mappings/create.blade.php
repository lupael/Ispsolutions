...
@section('title', 'Create Package Mapping')
...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Create Package Mapping</h1>
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <form action="{{ route('panel.admin.packages.mappings.store', $package) }}" method="POST">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="profile_name" id="profile_name" value="{{ old('profile_name') }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        @error('profile_name')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="speed_control_method" id="speed_control_method" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <option value="simple_queue" {{ old('speed_control_method') == 'simple_queue' ? 'selected' : '' }}>Simple Queue</option>
                            <option value="pcq" {{ old('speed_control_method') == 'pcq' ? 'selected' : '' }}>PCQ</option>
                            <option value="burst" {{ old('speed_control_method') == 'burst' ? 'selected' : '' }}>Burst</option>
                        </select>
