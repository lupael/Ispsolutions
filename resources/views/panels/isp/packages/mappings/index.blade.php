    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <div class="flex justify-between items-center">
                ...
                @can('create', $package)
                    <a href="{{ route('panel.isp.packages.mappings.create', $package) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        @canany(['update', 'delete'], $package)
                                            @can('update', $package)
                                                <a href="{{ route('panel.isp.packages.mappings.edit', [$package, $mapping]) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">Edit</a>
                                            @endcan
                                            @can('delete', $package)
                                                <form action="{{ route('panel.isp.packages.mappings.destroy', [$package, $mapping]) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this mapping?');">
                                                    @csrf
                                                    @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">
                                    ...
                                    @can('create', $package)
                                        <a href="{{ route('panel.isp.packages.mappings.create', $package) }}" class="text-indigo-600 hover:text-indigo-900">Add your first mapping</a>
                                    @endcan
                                </td>
                            </tr>
                        @endforelse
