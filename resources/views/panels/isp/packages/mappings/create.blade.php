...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <div class="flex justify-between items-center">
                ...
                <a href="{{ route('panel.isp.packages.mappings.index', $package) }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    Back to Mappings
                </a>
            </div>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <form action="{{ isset($mapping) ? route('panel.isp.packages.mappings.update', [$package, $mapping]) : route('panel.isp.packages.mappings.store', $package) }}" method="POST">
                @csrf
                @if(isset($mapping))
                    @method('PUT')
                @endif
