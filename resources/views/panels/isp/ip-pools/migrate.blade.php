...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            @if($pools->isEmpty() || $profiles->isEmpty())
                <!-- No data warning -->
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            ...
                            <div class="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
                                <p>Before you can migrate IP pools, you need to:</p>
                                <ul class="list-disc pl-5 mt-2 space-y-1">
                                    @if($pools->isEmpty())
                                        <li>Create at least 2 IP pools in your network configuration
                                            <a href="{{ route('panel.isp.network.ipv4-pools.create') }}" class="underline font-semibold">Create IP Pool</a>
                                        </li>
                                    @endif
                                    @if($profiles->isEmpty())
                                        <li>Create PPPoE profiles on your routers
                                            <a href="{{ route('panel.isp.network.pppoe-profiles') }}" class="underline font-semibold">Manage PPPoE Profiles</a>
                                        </li>
                                    @endif
                                </ul>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    validateBtn.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch('{{ route("panel.isp.ip-pools.migrate.validate") }}', {
                method: 'POST',
                headers: {
    ...
    startBtn.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch('{{ route("panel.isp.ip-pools.migrate.start") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('input[name="_token"]').value
            ...
            if (data.success) {
                ...
                progressLink.href = '{{ route("panel.isp.ip-pools.migrate.progress", ["migrationId" => "__ID__"]) }}'.replace('__ID__', data.migration_id);
