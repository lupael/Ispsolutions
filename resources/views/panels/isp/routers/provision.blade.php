        ...
        <div class="col-span-12">
            <div class="bg-white rounded-lg shadow">
                <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                    ...
                    <div>
                        <a href="{{ route('panel.isp.routers.provision.templates') }}" class="px-3 py-1 text-sm px-4 py-2 rounded border border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    testConnectionBtn.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch('{{ route("panel.isp.routers.provision.test-connection") }}', {
                method: 'POST',
                headers: {
    ...
    previewBtn.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch('{{ route("panel.isp.routers.provision.preview") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
    ...
    async function executeProvisioning() {
        ...
        try {
            const response = await fetch('{{ route("panel.isp.routers.provision.execute") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
    ...
    createBackupBtn.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch('{{ route("panel.isp.routers.provision.backup") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
    ...
    document.querySelectorAll('.rollback-btn').forEach(btn => {
        btn.addEventListener('click', async function() {
            ...
            try {
                const response = await fetch('{{ route("panel.isp.routers.provision.rollback") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
