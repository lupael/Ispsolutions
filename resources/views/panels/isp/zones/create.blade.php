...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Create Zone</h1>
                    <p class="mt-2 text-gray-600 dark:text-gray-400">Add a new geographic zone for customer organization</p>
                </div>
                <a href="{{ route('panel.isp.zones.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 transition">
    ...
    <!-- Create Form -->
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <form action="{{ route('panel.isp.zones.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="parent_id" id="parent_id"
                        ...
                        @foreach($parentZones as $parentZone)
                            <option value="{{ $parentZone->id }}" {{ old('parent_id') == $parentZone->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="coverage_type" id="coverage_type" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white sm:text-sm">
                        <option value="point" {{ old('coverage_type') == 'point' ? 'selected' : '' }}>Point Location</option>
                        <option value="radius" {{ old('coverage_type') == 'radius' ? 'selected' : '' }}>Radius Coverage</option>
                        <option value="polygon" {{ old('coverage_type') == 'polygon' ? 'selected' : '' }}>Polygon Area</option>
                    </select>
                    @error('coverage_type')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
            ...
            <div class="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                <a href="{{ route('panel.isp.zones.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    Cancel
                </a>
