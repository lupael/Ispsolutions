...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">{{ __('Send SMS') }}</h1>
                    <p class="mt-2 text-gray-600 dark:text-gray-400">{{ __('Send SMS message to customer') }}</p>
                </div>
                <div>
        ...
        <div class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $customer->phone ?? __('N/A') }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Email') }}</label>
                    <p class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $customer->email ?? __('N/A') }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- SMS Form -->
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <form action="{{ route('panel.isp.customers.send-sms.send', $customer) }}" method="POST" class="p-6">
            @csrf
            
            <div class="space-y-6">
                <!-- Template Selection -->
                <div>
                    <label for="template_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('SMS Template (Optional)') }}</label>
                    <select 
                        name="template_id" 
                        id="template_id" 
                        ...
                        <option value="">{{ __('Select a template (or write custom message below)') }}</option>
                    ...
                    <p class="mt-1 text-sm text-gray-500">{{ __('Choose a pre-configured template or write a custom message') }}</p>
                    @error('template_id')
                ...
                <div>
                    ...
                    <div class="mt-2 flex justify-between items-start">
                        ...
                        <div class="text-sm">
                            ...
                            <span class="text-gray-500">/ 500 {{ __('characters') }}</span>
                        </div>
                    </div>
