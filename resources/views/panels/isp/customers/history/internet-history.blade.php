...
@section('title', 'Internet History')
...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Internet Session History</h1>
                    <p class="mt-2 text-gray-600 dark:text-gray-400">
                        View all internet sessions for {{ $customer->username }}
                    </p>
                </div>
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <form method="GET" action="{{ route('panel.isp.customers.internet-history', $customer->id) }}">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
                    ...
                    <div>
                        <label for="session_type" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Session Type</label>
                        <select id="session_type" 
                                name="session_type" 
                                class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="all" {{ request('session_type') == 'all' ? 'selected' : '' }}>All</option>
                            <option value="pppoe" {{ request('session_type') == 'pppoe' ? 'selected' : '' }}>PPPoE</option>
                            <option value="hotspot" {{ request('session_type') == 'hotspot' ? 'selected' : '' }}>Hotspot</option>
                        </select>
                    </div>
                    <div class="flex items-end gap-2">
                        ...
                        <a href="{{ route('panel.isp.customers.internet-history', $customer->id) }}" 
                           class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            Clear
                        </a>
                    </div>
        ...
        <div class="p-6">
            <div class="flex justify-between items-center">
                ...
                <div class="flex gap-2">
                    <form method="POST" action="{{ route('panel.isp.customers.internet-history.export', $customer->id) }}" class="inline">
                        @csrf
                        <input type="hidden" name="format" value="csv">
                        <input type="hidden" name="start_date" value="{{ request('start_date') }}">
                        <input type="hidden" name="end_date" value="{{ request('end_date') }}">
                        <input type="hidden" name="session_type" value="{{ request('session_type') }}">
                        <button type="submit" 
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($sessions as $session)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900 dark:text-gray-100">
                                        {{ $session->stop_time ? $session->stop_time->format('Y-m-d H:i:s') : 'Active' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ $session->duration_formatted ?? '0m' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ $session->upload_formatted ?? '0 MB' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ $session->download_formatted ?? '0 MB' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ $session->total_formatted ?? '0 MB' }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-mono text-gray-900 dark:text-gray-100">
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="px-6 py-12 text-center">
                                    ...
                                    <p class="mt-2 text-gray-500 dark:text-gray-400">No session history found.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
