        ...
        <div class="p-6">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('panel.isp.customers.index') }}" class="inline-flex items-center px-4 py-2 bg-white/20 hover:bg-white/30 border border-white/30 rounded-lg font-medium text-sm text-white transition">
                    ...
                    @if(auth()->user()->operator_level <= 20 || auth()->user()->can('update', $customer))
                        <a href="{{ route('panel.isp.customers.edit', $customer->id) }}" class="inline-flex items-center px-4 py-2 bg-white hover:bg-gray-50 border border-transparent rounded-lg font-medium text-sm text-indigo-600 transition">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if(auth()->user()->operator_level <= 40)
                        <a href="{{ route('panel.isp.customers.advance-payment.create', $customer->id) }}" class="inline-flex items-center px-4 py-2 bg-green-500 hover:bg-green-600 border border-transparent rounded-lg font-medium text-sm text-white transition">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <a href="{{ route('panel.isp.customers.send-payment-link', $customer->id) }}" class="inline-flex items-center px-4 py-2 bg-blue-500 hover:bg-blue-600 border border-transparent rounded-lg font-medium text-sm text-white transition">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        @if(auth()->user()->operator_level <= 20 || auth()->user()->can('changePackage', $customer))
                            <a href="{{ route('panel.isp.customers.change-package.edit', $customer->id) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-purple-600 hover:bg-purple-700 border border-transparent rounded-lg font-medium text-sm text-white transition">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @if(auth()->user()->operator_level <= 20 || auth()->user()->can('generateBill', $customer))
                            <a href="{{ route('panel.isp.customers.bills.create', $customer->id) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-blue-600 hover:bg-blue-700 border border-transparent rounded-lg font-medium text-sm text-white transition">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                                Generate Bill
                        ...
                        @if(auth()->user()->operator_level <= 20 || auth()->user()->can('advancePayment', $customer))
                            <a href="{{ route('panel.isp.customers.other-payment.create', $customer->id) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-emerald-600 hover:bg-emerald-700 border border-transparent rounded-lg font-medium text-sm text-white transition">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        @if(auth()->user()->operator_level <= 20 || auth()->user()->can('sendSms', $customer))
                            <a href="{{ route('panel.isp.customers.send-sms', $customer->id) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-pink-600 hover:bg-pink-700 border border-transparent rounded-lg font-medium text-sm text-white transition">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @if(auth()->user()->operator_level <= 20 || auth()->user()->can('sendLink', $customer))
                            <a href="{{ route('panel.isp.customers.send-payment-link', $customer->id) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-cyan-600 hover:bg-cyan-700 border border-transparent rounded-lg font-medium text-sm text-white transition">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        ...
                        @if(auth()->user()->operator_level <= 20 || auth()->user()->can('changeOperator', $customer))
                            <a href="{{ route('panel.isp.customers.change-operator.edit', $customer->id) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 border border-transparent rounded-lg font-medium text-sm text-white transition">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
                                </svg>
                                Change Operator
...
<script nonce="{{ $cspNonce ?? '' }}">
    ...
    function refreshUsage() {
        ...
        // Fetch usage data
        fetch(`{{ route('panel.isp.customers.check-usage', ['customer' => $customer->id]) }}`)
        ...
        document.querySelectorAll('.action-button[data-action]').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (confirm(`Are you sure you want to ${action} this customer?`)) {
                    ...
                    fetch(`{{ route('panel.isp.customers.index') }}/${customerId}/${action}`, {
                        method: 'POST',
                        headers: {
