        ...
        <div class="p-6">
            <div class="flex justify-between items-center">
                ...
                <div class="flex space-x-2">
                    <a href="{{ route('panel.isp.cable-tv.packages.index') }}" class="inline-flex items-center px-4 py-2 bg-purple-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-purple-700 transition">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                        </svg>
                        Packages
                    </a>
                    <a href="{{ route('panel.isp.cable-tv.channels.index') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 transition">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                        </svg>
                        Channels
                    </a>
                    <a href="{{ route('panel.isp.cable-tv.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 transition">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @forelse($subscriptions as $subscription)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('panel.isp.cable-tv.edit', $subscription) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 mr-3">Edit</a>
                                    @if($subscription->status === 'active')
                                        <button data-action="suspend" data-id="{{ $subscription->id }}" data-url="{{ route('panel.isp.cable-tv.suspend', $subscription) }}" class="confirm-action-btn text-yellow-600 hover:text-yellow-900 dark:text-yellow-400 dark:hover:text-yellow-300 mr-3">Suspend</button>
                                    @elseif($subscription->status === 'suspended')
                                        <form action="{{ route('panel.isp.cable-tv.reactivate', $subscription) }}" method="POST" class="inline">
                                            @csrf
                                    ...
                                    @endif
                                    <form action="{{ route('panel.isp.cable-tv.destroy', $subscription) }}" method="POST" class="inline delete-cable-tv-form">
                                        @csrf
                                        @method('DELETE')
...
document.addEventListener('click', function(e) {
    const target = e.target.closest('.confirm-action-btn');
    if (target) {
        const action = target.getAttribute('data-action');
        const id = target.getAttribute('data-id');
        
        if (action === 'suspend') {
            // Collect suspension reason
            const reason = window.prompt('Please provide a reason for suspending this subscription:');
            if (reason === null) {
            ...
            if (!actionUrl) {
                console.error('No suspend URL provided on the suspend button.');
                alert('Unable to perform suspend action: no target URL configured.');
                return;
            }
            ...
            // Build and submit a POST form with the required reason parameter
