                        ...
                        @forelse($operators as $operator)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                    <a href="{{ route('panel.isp.operators.assign-sms-rate', $operator->id) }}" class="inline-flex items-center px-3 py-1 {{ $operator->smsRate ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-indigo-600 hover:bg-indigo-700' }} text-white rounded-md">
                                    ...
                                    @if($operator->smsRate)
                                        <form action="{{ route('panel.isp.operators.delete-sms-rate', $operator->id) }}" method="POST" class="inline delete-sms-rate-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="inline-flex items-center px-3 py-1 bg-red-600 text-white rounded-md hover:bg-red-700">
                                                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
// Confirm delete for SMS rates
document.querySelectorAll('.delete-sms-rate-form').forEach(form => {
    form.addEventListener('submit', function(e) {
        if (!confirm('Are you sure you want to remove this SMS rate?')) {
            e.preventDefault();
        }
    });
});
