                ...
                @forelse($operators as $operator)
                    <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                        <div class="flex justify-between items-start mb-4">
                            ...
                            <a href="{{ route('panel.isp.operators.assign-package-rate', $operator->id) }}" class="inline-flex items-center px-3 py-1.5 bg-indigo-600 text-white text-sm rounded-md hover:bg-indigo-700">
                                        ...
                                        @foreach($operator->packageRates as $rate)
                                            <tr>
                                                ...
                                                <td class="px-4 py-3 text-sm">
                                                    <form action="{{ route('panel.isp.operators.delete-package-rate', [$operator->id, $rate->package_id]) }}" method="POST" class="inline delete-rate-form">
                                                        @csrf
                                                        @method('DELETE')
...
// Confirm delete for package rates
document.querySelectorAll('.delete-rate-form').forEach(form => {
    form.addEventListener('submit', function(e) {
        if (!confirm('Are you sure you want to remove this package rate?')) {
            e.preventDefault();
        }
    });
});
