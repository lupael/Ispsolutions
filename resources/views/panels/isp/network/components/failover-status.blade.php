...
function failoverStatus() {
    return {
        ...
        async configureFailover() {
            ...
            this.isLoading = true;
            try {
                const response = await fetch('{{ route("panel.isp.routers.failover.configure", $router->id) }}', {
                    method: 'POST',
                    headers: {
        ...
        async switchMode(mode) {
            ...
            this.isLoading = true;
            try {
                const radiusUrl = '{{ route("panel.isp.routers.failover.switch-to-radius", $router->id) }}';
                const routerUrl = '{{ route("panel.isp.routers.failover.switch-to-router", $router->id) }}';
        ...
        async testRadius() {
            this.isLoading = true;
            try {
                const response = await fetch('{{ route("panel.isp.routers.failover.test-connection", ["routerId" => $router->id]) }}', {
                    method: 'POST',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'Accept': 'application/json',
