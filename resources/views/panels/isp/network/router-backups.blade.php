                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                ...
                                @if($backup->file_path)
                                <a href="{{ route('panel.isp.network.routers.backup.download', [$router->id, $backup->id]) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 mr-3">
                                    Download
                                </a>
                                @endif
...
function routerBackups() {
    return {
        ...
        async createBackup() {
            ...
            try {
                const response = await fetch('{{ route('panel.isp.routers.backup.create', ['routerId' => $router->id]) }}', {
                    method: 'POST',
                    headers: {
        ...
        async restoreBackup(backupId) {
            ...
            try {
                const response = await fetch('{{ route('panel.isp.routers.backup.restore', ['routerId' => $router->id]) }}', {
                    method: 'POST',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'Accept': 'application/json',
        ...
        async deleteBackup(backupId) {
            ...
            try {
                const url = '{{ route('panel.isp.routers.backup.destroy', ['routerId' => $router->id, 'backupId' => '__BACKUP_ID__']) }}';
