    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <div class="flex justify-between items-center">
                ...
                <a href="{{ route('panel.isp.network.ipv4-pools.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                        ...
                        @forelse($pools as $pool)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('panel.isp.network.ipv4-pools.edit', $pool->id) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 mr-3">View / Edit</a>
                                    <form action="{{ route('panel.isp.network.ipv4-pools.destroy', $pool->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this IPv4 pool?');">
                                        @csrf
                                        @method('DELETE')
        ...
        actionButton.addEventListener('click', function() {
            ...
            if (action === 'delete') {
                if (confirm(`Are you sure you want to delete ${selectedIds.length} IP pool(s)? This action cannot be undone.`)) {
                    ...
                    form.method = 'POST';
                    form.action = '{{ route("panel.isp.network.ipv4-pools.bulk-delete") }}';
