        ...
        <div class="p-6">
            <div class="flex justify-between items-center">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('panel.isp.master-packages.edit', $masterPackage) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">
                        Edit
                    </a>
                    <a href="{{ route('panel.isp.master-packages.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700">
                        Back
                    </a>
                </div>
            </div>
                ...
                <div>
                    ...
                    <div class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                        @if($masterPackage->pppoeProfile)
                            <a href="{{ route('panel.isp.network.pppoe-profiles') }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">
    ...
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('panel.isp.master-packages.assign', $masterPackage) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">
                    Assign to Operator
                </a>
            </div>
                        ...
                        @forelse($masterPackage->operatorRates as $rate)
                            ...
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                    @if($rate->operator)
                                        <a href="{{ route('panel.isp.operators.profile', $rate->operator->id) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">
                                            {{ $rate->operator->name }}
                                        </a>
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <form action="{{ route('panel.isp.master-packages.remove-operator', [$masterPackage, $rate]) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
