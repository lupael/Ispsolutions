...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
    <div class="mb-8">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Create SMS Gateway</h1>
                <p class="mt-2 text-gray-600 dark:text-gray-400">Add a new SMS gateway integration</p>
            </div>
            <a href="{{ route('panel.developer.gateways.sms') }}" class="bg-gray-600 hover:bg-gray-700 text-white font-semibold py-2 px-4 rounded-lg">
                Back to List
            </a>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <form action="{{ route('panel.developer.gateways.sms.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="provider" id="provider" required 
                    ...
                    <option value="nexmo" {{ old('provider') == 'nexmo' ? 'selected' : '' }}>Nexmo (Vonage)</option>
                    <option value="msg91" {{ old('provider') == 'msg91' ? 'selected' : '' }}>Msg91</option>
                    <option value="bulksms" {{ old('provider') == 'bulksms' ? 'selected' : '' }}>BulkSMS</option>
