...
## Table of Contents
1. [Onboarding Process Overview](#1-onboarding-process-overview)
2. [Minimum Configuration Requirements](#2-minimum-configuration-requirements)
3. [Adding a Router](#3-adding-a-router)
4. [Router Configuration](#4-router-configuration)
5. [Router Configuration Tasks](#5-router-configuration-tasks)
6. [Scheduler & Sync Operations](#6-scheduler--sync-operations)
7. [RADIUS Server Responsibilities](#7-radius-server-responsibilities)
8. [Import Operations](#8-import-operations)
9. [Form Fields Reference](#9-form-fields-reference)
...
- **Check:** `billing_profile::where('operator_id', $operator->id)->count() > 0`
...
- **Applies to:** Each operator under Admin
...
**File:** `app/Console/Commands/AdminImportCommand.php`
...
class RouterConfigurationTest extends TestCase
{
    ...
    public function test_router_can_be_configured()
    {
        $router = nas::factory()->create([
            'operator_id' => $this->Admin->id,
        ]);

        $response = $this->actingAs($this->Admin)
            ->post("/routers/{$router->id}/configuration", [
                'operator_id' => $this->Admin->id,
                'change_system_identity' => 'yes',
            ]);
...
1. **RADIUS Secret:** Must be at least 16 characters long
2. **API Credentials:** Store securely, never expose in logs
3. **Database Connections:** Use separate credentials for RADIUS database
4. **Router Access:** Restrict API access to specific IP addresses
5. **SNMP Community:** Use non-default community strings
6. **Backup Settings:** Configure backup routers for failover

### Performance Optimization

1. **Connection Pooling:** Reuse API connections when possible
2. **Batch Operations:** Use bulk inserts for imports
3. **Caching:** Cache router configurations and package data
...
- **NAS:** Network Access Server (Router)
...
- **Operator:** ISP or reseller managing customers
- **Admin:** Primary account owner with full access
